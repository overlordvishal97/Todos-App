...
data['month_num'] = data['index'].apply(lambda x: month_map[str(x).split(',')[0]] if str(x).split(',')[0] in month_map.keys() else None)
...
melted = pd.melt(data, id_vars=['month_num', 'final'], value_vars=['within_month', 'within_2month', 'within_3month', 'within_4month', 'within_5month', 'within_6month', 'within_7month'], var_name='months', value_name='value')


# Convert the months column to numerical values
melted['months'] = melted['months'].apply(lambda x: int(x.split('n_')[1].replace('month', '')))
